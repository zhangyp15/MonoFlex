...
class KittiRepeatFactorTrainingSampler(Sampler):
    ...
    def _get_repeat_factors(self, dataset, repeat_thresh):
        ...
        #    r(I) = max_{c in I} r(c)
        rep_factors = []
        for lable_file in dataset.label_files:
            labelpath = os.path.join( dataset.label_dir , lable_file)
            objs = read_label(labelpath)
            cls_ids = []
            for obj in objs:
                cls_id = TYPE_ID_CONVERSION[obj.type]
                if cls_id < 0 or cls_id > 2: continue
                cls_ids.append(category_rep[cls_id])
            cls_id_max = max(cls_ids)  if cls_ids !=[] else 1.0
            rep_factors.append(cls_id_max)
            



        # for per_data in dataset:
        #     rep_factor = max(per_data[1].get_field("reg_weight"))
        #     rep_factors.append(rep_factor)
